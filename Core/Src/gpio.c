...
/* USER CODE BEGIN 0 */
uint32_t keyScan_TS[keyNum];
keyState_t keyState[keyNum];
/* USER CODE END 0 */
    ...
    while (i--)
    {
        if ((keyInfo & (1 << i))) /* KeyDown */
        {
            ...
            case S0:
                ...
                keyScan_TS[i] = HAL_GetTick(); // set time counter
                break;
        ...
        else if (!(keyInfo & (1 << i))) /* KeyUp */
        {
            ...
            /* KeyDown State */
            case S1:
                if (HAL_GetTick() - (keyScan_TS[i]) >= keyLongPressTime)       // check long press
                    keyState[i] = S4;                                          // update to Long
                else if (HAL_GetTick() - (keyScan_TS[i]) >= keyShortPressTime) // check short press
